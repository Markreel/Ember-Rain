...
public class PlayerBehaviour : MonoBehaviour
{
    ...
    {
        if ((Input.GetButtonDown("Jump") || Input.GetMouseButtonDown(0)) && CurrentState == States.Running)
        {
            Jump();
        }
        ...
        if ((Input.GetButtonUp("Jump") || Input.GetMouseButtonUp(0)) && (CurrentState == States.Flying || CurrentState == States.Gliding))
        {
    ...
    {
        if (CurrentState == States.Gliding && yVelocity < 0.1f) { anim.SetBool("IsGliding", true); }
    ...
    private void FallDownGameOver()
    {
        ...
        float xVelocity = Mathf.Abs(transform.position.x) > 1 ? 0.02f * Mathf.Abs(transform.position.x) : 0.02f;
    ...
    private void UpdatePosition()
    {
        ...
        transform.position = new Vector3(Mathf.Clamp(transform.position.x, -500, -0.25f), yPos, transform.position.z);
